  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            "relative flex flex-col rounded-xl bg-background shadow-md py-1.5 px-2.5",
            isDragging
              ? "border border-dashed border-emerald-400"
              : "border border-[#EC6341]/40 focus-within:border-[#EC6341]/70 transition-colors",
          )}
        >
          {isDragging && (
...
const MessageInputPlainTextarea = ({
  ...
  return (
    <textarea
      ...
      onPaste={handlePaste}
      className={cn(
        "flex-1 px-1 py-1.5 rounded-t-lg bg-background text-foreground resize-none text-sm min-h-8 max-h-[40vh] focus:outline-none placeholder:text-muted-foreground/50",
        className,
      )}
...
>(({ className, children, ...props }, ref) => {
  ...
  const buttonClasses = cn(
    "w-8 h-8 bg-foreground text-background rounded-lg hover:bg-foreground/90 disabled:opacity-50 flex items-center justify-center enabled:cursor-pointer",
    className,
  );
...
>(({ className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "flex justify-between items-center mt-1 p-0.5 gap-1.5",
        className,
      )}
